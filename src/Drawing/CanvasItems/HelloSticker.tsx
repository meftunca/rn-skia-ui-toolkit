import type {SkMatrix, SharedValueType} from '@shopify/react-native-skia';
import {fitbox, Group, LinearGradient, Path, rect, Skia} from '@shopify/react-native-skia';
...
interface HelloStickerProps {
  matrix: SharedValueType<SkMatrix>;
}

export const HelloSticker = ({matrix}: HelloStickerProps) => {
  const bounds = path.computeTightBounds();
  const HelloStickerDimensions = rect(60, 600, 300, (300 * bounds.height) / bounds.width);
  return (
    <Group matrix={matrix}>
      <Group transform={fitbox('contain', bounds, HelloStickerDimensions)}>
        ...
        />
        <Path path={path} color="white" style="stroke" strokeWidth={54} strokeCap="round" strokeJoin="round" />
